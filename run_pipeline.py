...
def gen_adrd_output_df(df):
    ...
    df.loc[~df['child_concept_cat'].isnull(), 'SDoH_attributes'] = (
        df.loc[~df['child_concept_cat'].isnull()]
          .apply(lambda row: f"{row['child_concept_cat']}: {row['child_concept_value']}", axis=1)
        )
...
class BatchProcessor(object):
    ...
    def get_subdirs(self, p):
        sub_ps = [
            ...
            if x.is_dir()
            and x.stem not in OUTPUT_DIR
            and ".ipynb_checkpoints" not in x.name
        ]
...
def sequential_process(experiment_info):
    ...
    pipeline.run()
    pr.disable()
    pr.dump_stats(Path(experiment_info['root_dir']) / f'logs/SDoH_HepC_notes_sample_RHEL8.prof')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gpu_nodes", nargs="+", default="-1", help="gpu_device_id")
    ...
    if len(args.gpu_nodes) > 1:
        print('MultiGPU support enabled')
        print(args)
        ...
        experiment_info['device'] = torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")
        print("==========START==========")
